...
**getGoalMeta(uint256 id) → (address owner, string title, uint64 createdAt, bool isCompleted)**
- Returns public metadata for a goal
- View function, no gas cost

**getEncryptedDescription(uint256 id) → bytes**
- Returns encrypted description bytes
- Client-side decryption required

**getEncryptedDeadline(uint256 id) → euint64**
- Returns FHE-encrypted deadline
- Requires FHEVM decryption

**getEncryptedPriority(uint256 id) → euint8**
- Returns FHE-encrypted priority
- Requires FHEVM decryption

**getEncryptedProgress(uint256 id) → euint8**
- Returns FHE-encrypted progress
- Requires FHEVM decryption

**getGoalIdsByOwner(address owner) → uint256[]**
- Returns array of goal IDs for an owner
- View function

**getGoalCountByOwner(address owner) → uint256**
